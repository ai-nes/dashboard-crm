...
- `priorityQueue` chỉ là danh sách tối đa 3 hồ sơ ưu tiên. Các sheet phải lấy danh sách theo nhóm từ `dashboard.detailRecords`, không dùng lại `priorityQueue` cho mọi metric.

## Nguồn triển khai

- Backend: `crm/api/lead_sale.py`, phần xây dựng stage, action, `detailRecords` và `priorityQueue`.
