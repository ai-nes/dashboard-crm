...
export default function WithLayout({ children }: { children: ReactNode }) {
  ...
  const isSegmentDetailPage = /^\/(director\/admin\/segments|sale\/(segments|next-best-action)|lead-sale\/segments|ctv-sale\/segments)\/[^/]+\/?$/.test(pathname);
            ...
            >
              <div
                className={cn(
                  ...
                  isChatbotPage
                    ? "h-full max-w-none"
                    : isTaskManagementPage || isSegmentDetailPage || isSegmentManagementPage
                      ? "h-full max-w-384"
                      : "max-w-384 pb-5",
                )}
              >
