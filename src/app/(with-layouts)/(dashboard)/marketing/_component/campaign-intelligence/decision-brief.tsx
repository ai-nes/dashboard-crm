...
export function DecisionBrief({ recommendation }: Pick<CampaignIntelligenceResponse, "recommendation">) {
  return (
    <Card className="h-fit p-0 xl:sticky xl:top-5">
      <CardHeader className="border-b border-card-border px-5 py-4">
        <div>
          ...
          <p className="mt-0.5 text-xs text-text-tertiary">Đề xuất dựa trên attribution đã đối soát</p>
        </div>
        <Badge color="success">Độ tin cậy cao</Badge>
      </CardHeader>
      <div className="space-y-5 p-5">
        <div>
          <p className="text-xs font-medium text-text-tertiary">Hành động đề xuất</p>
        ...
        <div className="border-y border-card-border py-4">
          <p className="text-xs font-medium text-text-tertiary">Tác động doanh thu ước tính</p>
          <p className="mt-1 text-2xl font-semibold tracking-[-0.3px] text-success-500">
            +{formatCurrency(recommendation.impact)}
          </p>
          <p className="mt-1 text-xs text-text-tertiary">trong 30 ngày tiếp theo</p>
        </div>
        <div>
        ...
        <Button className="w-full" onPress={() => toast.success("Đã tạo kịch bản tái phân bổ ngân sách.")}>
          Tạo kịch bản phân bổ
        </Button>
        <p className="text-xs leading-5 text-text-tertiary">
          Doanh thu xác nhận chỉ gồm enrollment đã được đối soát trong CRM.
        </p>
      </div>
    </Card>
  );
