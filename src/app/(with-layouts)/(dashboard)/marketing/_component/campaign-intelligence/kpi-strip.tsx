...
export function KpiStrip({ summary }: Pick<CampaignIntelligenceResponse, "summary">) {
  const items = [
    { label: "Ngân sách", value: formatCompactCurrency(summary.spend), change: "+5,7%" },
    { label: "Qualified leads", value: formatNumber(summary.qualifiedLeads), change: "+12,7%" },
    { label: "Hồ sơ", value: formatNumber(summary.applications), change: "+10,3%" },
    { label: "Nhập học", value: formatNumber(summary.enrollments), change: "+8,3%" },
    { label: "Doanh thu xác nhận", value: formatCompactCurrency(summary.confirmedRevenue), change: "+15,6%" },
  ...
  return (
    ...
    >
      {items.map((item) => (
        <div key={item.label} className="min-w-0 p-4 transition-colors hover:bg-background-soft-50/40">
          <p className="truncate text-xs font-medium text-text-tertiary">{item.label}</p>
          ...
          <div className="mt-1.5 flex items-center gap-1 text-xs font-semibold text-success-500">
            ...
            <span className="truncate font-normal text-text-tertiary">kỳ trước</span>
          </div>
        </div>
      ))}
    </section>
