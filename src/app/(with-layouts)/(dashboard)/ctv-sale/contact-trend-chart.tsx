  ...
  return (
    <div className="rounded-lg border border-card-border bg-card-background p-3 text-xs shadow-md">
      ...
      {payload.map((item) => (
        <p
          key={item.name}
          className="flex items-center justify-between gap-6 py-0.5 text-text-secondary"
        >
...
export default function ContactTrendChart({ data }: ContactTrendChartProps) {
  const [range, setRange] = useState<CtvSaleRange>(data.defaultRange);
  ...
  return (
    <Card className="min-w-0 p-5 sm:p-6">
      <CardHeader className="items-start gap-3">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-text-tertiary">
            Số lượt liên hệ và kết nối thành công.
          </p>
        </div>
        <Select
          value={range}
          onChange={(value) => setRange(value as CtvSaleRange)}
      ...
      <div
        className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-[11px] text-text-secondary"
      ...
      <div
        className="mt-1 h-56 w-full"
        role="img"
        aria-label={`Biểu đồ xu hướng liên hệ trong ${range === "7d" ? "7 ngày" : "30 ngày"}`}
      >
        <ChartContainer
