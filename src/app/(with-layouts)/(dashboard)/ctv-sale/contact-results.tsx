...
import type { CtvSaleContactOutcomes } from "@/services/api/ctv-sale";

import { outcomeColor } from "./_components/data";

interface ContactResultsProps {
  data: CtvSaleContactOutcomes;
}

export default function ContactResults({ data }: ContactResultsProps) {
    ...
    <Card className="min-w-0 p-5 sm:p-6">
      <CardHeader className="items-start">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-text-tertiary">
            Hiệu quả từ {data.total} lượt cập nhật gần nhất.
          </p>
        </div>
        <span className="inline-flex items-center gap-1 rounded-full bg-badge-success-background px-2.5 py-1 text-[11px] font-semibold text-success-600">
          ...
          {data.connectedRate === null ? "—" : `${data.connectedRate}% kết nối`}
        </span>
      </CardHeader>
      ...
      <div className="mt-6 space-y-4">
        {data.items.length === 0 ? (
          <p className="text-sm text-text-tertiary">
            Chưa có kết quả liên hệ trong khoảng thời gian này.
          </p>
        ) : (
          data.items.map((item) => {
            const color = outcomeColor(item);

            return (
              <div
                key={item.id}
                className="grid grid-cols-[7.5rem_minmax(0,1fr)_2.5rem] items-center gap-3 text-xs sm:grid-cols-[9rem_minmax(0,1fr)_3rem] sm:gap-4"
              >
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-card-border pt-4">
        <p className="text-xs text-text-tertiary">
          Tóm tắt theo snapshot dữ liệu hiện tại.
        </p>
        <Link
