...
import type { CtvSaleKpi } from "@/services/api/ctv-sale";

import { KPI_PRESENTATION } from "./data";
...
function StatCard({ stat }: StatCardProps) {
  ...
  const trend =
    stat.deltaValue === null
      ? null
      : `${stat.deltaValue > 0 ? "+" : ""}${stat.deltaValue}${stat.deltaUnit === "percent" ? "%" : ""}`;
  ...
  return (
    <Card className="group flex h-fit min-w-0 flex-col justify-start gap-4 p-4 transition duration-200 hover:-translate-y-0.5 hover:shadow-md sm:p-5">
      <div className="flex items-start justify-between gap-3">
        ...
        {trend ? (
          <span
            className={`inline-flex items-center gap-1 text-[11px] font-medium ${styles.trend}`}
          >
      ...
      <div>
        ...
        <p
          className={`mt-1 text-[28px] leading-8 font-semibold tracking-[-0.6px] ${styles.value}`}
        >
          {stat.value}
        </p>
