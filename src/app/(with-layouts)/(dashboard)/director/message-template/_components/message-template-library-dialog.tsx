...
} from "./message-template-library-data";
...
function findTemplate(templateId: string): MessageTemplateItem {
  return (
    messageTemplateGroups
      .flatMap((group) => group.templates)
      .find((template) => template.id === templateId) ??
    messageTemplateGroups[0].templates[0]
  );
}
...
export default function MessageTemplateLibraryDialog({
  ...
  const firstTemplateId = messageTemplateGroups[0].templates[0].id;
  const [selectedTemplateId, setSelectedTemplateId] = useState(firstTemplateId);
  const [previewedTemplateId, setPreviewedTemplateId] = useState(firstTemplateId);
  const previewedTemplate = useMemo(
    () => findTemplate(previewedTemplateId),
    [previewedTemplateId],
  );
  const totalTemplateCount = messageTemplateGroups.reduce(
    ...
    <Backdrop isOpen={isOpen} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="grid min-h-0 flex-1 grid-rows-[minmax(16rem,0.85fr)_minmax(20rem,1.15fr)] lg:grid-cols-[minmax(22rem,0.78fr)_minmax(0,1.22fr)] lg:grid-rows-1">
          ...
          >
            <div role="listbox" aria-label="Các mẫu tin nhắn" className="min-h-0 flex-1 overflow-y-auto px-3 py-4 sm:px-4 sm:py-5">
              <div className="space-y-6">
                {messageTemplateGroups.map((group) => (
                  <section key={group.id} aria-labelledby={`${group.id}-heading`}>
                    <div className="flex items-center gap-2 border-b border-card-border pb-2.5">
                      <span className="rounded-md bg-background-gray-secondary px-1.5 py-0.5 text-xs font-semibold tabular-nums text-text-tertiary">
                        {String(messageTemplateGroups.indexOf(group) + 1).padStart(2, "0")}
                      </span>
                      <h2
                        id={`${group.id}-heading`}
          ...
          <section
            aria-live="polite"
            aria-label={`Nội dung mẫu ${previewedTemplate.title}`}
              ...
              <div className="mx-auto max-w-3xl">
                <dl className="space-y-6 text-base">
                  <div>
                    ...
                    <dd className="mt-1 text-lg font-semibold leading-7 text-text-primary">{previewedTemplate.title}</dd>
                  </div>
                  <div>
        ...
        <footer className="flex shrink-0 flex-col items-stretch gap-3 border-t border-card-border bg-background-white-primary px-5 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-7">
          <span className="min-w-0 truncate text-xs text-text-tertiary">
            {totalTemplateCount} mẫu có sẵn · Đang xem: <span className="font-medium text-text-secondary">{previewedTemplate.title}</span>
          </span>
