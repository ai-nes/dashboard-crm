...
export default function SegmentGuardrails() {
  return (
    <Card className="min-w-0 overflow-hidden bg-background-gray-primary p-0">
      <CardHeader className="border-b border-card-border p-5"><div><div className="flex flex-wrap items-center gap-2"><CardTitle>Chất lượng & ràng buộc dữ liệu</CardTitle><Badge color="success"><Shield1Check size={13} aria-hidden="true" />Ngưỡng 30 hồ sơ</Badge></div><p className="mt-1 text-xs leading-5 text-text-tertiary">Các điều kiện của M-04 được kiểm soát ngay tại nơi phân tích.</p></div></CardHeader>
      ...
      <div className="flex items-start gap-2 border-t border-card-border bg-card-background px-5 py-3 text-xs leading-5 text-text-tertiary"><InfoTriangle size={15} className="mt-0.5 shrink-0 text-warning-500" aria-hidden="true" />Các tiêu chí bị khóa vẫn hiển thị để người dùng hiểu vì sao chưa thể sử dụng, thay vì bị ẩn khỏi giao diện.</div>
    </Card>
  );
}
