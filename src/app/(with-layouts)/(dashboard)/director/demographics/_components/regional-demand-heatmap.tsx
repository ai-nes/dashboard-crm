      ...
      <CardHeader className="border-b border-card-border p-5">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-text-tertiary">
            {isCountMetric ? "Số học sinh theo ngành và địa bàn." : "Chỉ số tương đối theo ngành; không phải số học sinh."}
          </p>
        </div>
        <span className="text-xs text-text-tertiary">{isCountMetric ? "Số học sinh" : "Chỉ số 0–100"}</span>
      </CardHeader>
      {hasData ? (
        <>
        ...
        <div className="p-4 sm:p-5">
          <ChartEmptyState message="Chưa có dữ liệu quan tâm theo địa bàn" detail="Cần dữ liệu học sinh theo ngành và địa bàn để hiển thị bảng nhiệt." />
        </div>
      )}
    </Card>
  );
