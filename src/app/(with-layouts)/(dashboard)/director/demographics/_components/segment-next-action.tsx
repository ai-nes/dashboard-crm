...
export default function SegmentNextAction({ segment }: SegmentNextActionProps) {
  const topChannel = [...segment.channels].sort((first, second) => second.value - first.value)[0];
  const isHighPriority = segment.opportunityScore >= 85;
  const primaryAction = isHighPriority ? "Ưu tiên tiếp cận sớm" : "Thử nghiệm quy mô nhỏ";
  const primaryDescription = isHighPriority
  ...
  return (
    <Card className="min-w-0 overflow-hidden border-primary-200 bg-card-background">
      <CardHeader className="border-b border-card-border p-5">
        ...
        <Badge color={isHighPriority ? "success" : "primary"}>{isHighPriority ? "Ưu tiên cao" : "Cần thử nghiệm"}</Badge>
      </CardHeader>
      <div className="p-5">
        ...
        <div className="mt-5 space-y-4">
          <ActionStep number="1" title={`Tiếp cận qua ${topChannel.name}`} detail={`${topChannel.value}% tương tác đầu tiên đến từ kênh này.`} />
          <ActionStep number="2" title="Tổ chức một hoạt động tư vấn" detail="Ưu tiên Career Talk hoặc tư vấn nhóm nhỏ." />
          <ActionStep number="3" title="Đánh giá lại sau 30 ngày" detail="So sánh số học sinh được tiếp cận và số hồ sơ nhập học." />
        </div>
      </div>
    </Card>
  );
...
function ActionStep({ number, title, detail }: ActionStepProps) {
  return (
    <div className="flex items-start gap-3">
      <span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-badge-primary-background text-xs font-semibold text-badge-primary-text">{number}</span>
