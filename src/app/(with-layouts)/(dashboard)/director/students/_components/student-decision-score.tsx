...
export default function StudentDecisionScore({
  data,
}: StudentDecisionScoreProps) {
  ...
  return (
    <section
      className="min-w-0 p-5 lg:flex lg:h-full lg:flex-col lg:p-6"
      aria-labelledby="student-score-heading"
    >
      <div className="flex flex-wrap items-center gap-2">
        <h3
          id="student-score-heading"
          className="text-lg font-semibold text-text-primary"
        >
          Đánh giá học sinh này
        </h3>
        <Badge color={confirmed ? "success" : "warning"}>
          ...
          {data.classification.reviewStatus || "Chờ xác nhận"}
        </Badge>
      </div>
      ...
      <div className="mt-5 grid min-w-0 gap-0 lg:flex-1 lg:grid-cols-[11rem_minmax(0,1fr)] lg:items-stretch">
        <div className="flex flex-col items-center justify-center text-center lg:border-r lg:border-card-border lg:pr-6">
          ...
          <p className="mt-2 text-xs text-text-tertiary">
            Điểm càng cao càng nên chăm sóc sớm
          </p>
          <p
            className={`mt-3 text-xs font-semibold ${(data.insight.scoreDelta ?? 0) >= 0 ? "text-success-500" : "text-error-500"}`}
          >
            {data.insight.scoreDelta != null
              ? `${data.insight.scoreDelta >= 0 ? "+" : ""}${data.insight.scoreDelta} gần nhất`
              : "-"}
          </p>
        </div>
        ...
        <div className="min-w-0 lg:flex lg:flex-col lg:pl-6">
          ...
          <div className="h-32 min-h-32 w-full lg:min-h-0 lg:flex-1">
            <ChartContainer
              width="100%"
              height="100%"
              minWidth={0}
            ...
            >
              ...
              >
                ...
                <Bar
                  ...
                  barSize={28}
                >
                  {scoreProfile.map((item, index) => (
                    <Cell key={item.label} fill={scoreColors[index]} />
                  ))}
                  <LabelList
                    dataKey="value"
                    position="right"
