...
export default function JourneyTimeline({ data }: Student360SectionProps) {
  const completedCount = data.journey.filter((event) => event.status === "completed").length;
  ...
  return <Card className="flex min-w-0 flex-col p-5"><CardHeader className="mb-5"><div><CardTitle>Điểm chạm & hành trình</CardTitle><p className="mt-1 text-sm leading-6 text-text-tertiary">Thanh giai đoạn cho biết vị trí chuẩn hóa; danh sách dưới là các điểm chạm theo thời gian.</p></div><Badge color="success">{completedCount} điểm chạm đã ghi nhận</Badge></CardHeader><div className="mb-5 overflow-x-auto rounded-xl border border-primary-200 bg-badge-primary-background p-3"><p className="text-[11px] font-semibold tracking-wide text-badge-primary-text uppercase">Giai đoạn chuẩn hóa</p><ol className="mt-3 flex min-w-[42rem] items-start" aria-label="Bảy giai đoạn hành trình tuyển sinh">{journeyStages.map((stage, index) => { const complete = index < currentStageIndex; const current = index === currentStageIndex; return <li key={stage} className="flex min-w-0 flex-1 items-start"><div className="flex min-w-0 flex-col items-center text-center"><span className={`flex size-7 items-center justify-center rounded-full text-xs font-semibold ${complete ? "bg-badge-success-background text-success-500" : current ? "bg-primary-500 text-primary-text" : "border border-card-border bg-card-background text-text-tertiary"}`} aria-current={current ? "step" : undefined}>{complete ? "✓" : index + 1}</span><span className={`mt-2 max-w-24 text-[10px] leading-4 ${current ? "font-semibold text-primary-500" : "text-text-tertiary"}`}>{stage}</span></div>{index < journeyStages.length - 1 && <span className={`mt-3 h-px flex-1 ${index < currentStageIndex ? "bg-success-500/50" : "bg-card-border"}`} aria-hidden="true" />}</li>; })}</ol></div><ol className="flex flex-col" aria-label="Toàn bộ điểm chạm trong hành trình tuyển sinh">{data.journey.map((event) => {
    const tone = channelTone[event.channel];
    return <li key={event.id} className="relative grid grid-cols-[5.75rem_minmax(0,1fr)] gap-3 pb-4 last:pb-0 sm:grid-cols-[7.25rem_minmax(0,1fr)]"><span className={`break-words pt-1 text-xs font-semibold ${event.status === "current" ? "text-brand-500" : "text-text-tertiary"}`}>{event.date}</span><div className="relative min-w-0 pl-6"><span className={`absolute top-1 left-0 z-10 size-3 rounded-full ring-4 ring-card-background ${tone.dot}`} aria-hidden="true" />{event.status !== "current" && <span className="absolute top-3 bottom-[-16px] left-1.5 w-px bg-card-border last:hidden" aria-hidden="true" />}<div className={event.status === "current" ? "rounded-xl border border-primary-200 bg-badge-primary-background p-4" : "border-b border-card-border pb-4"}><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-semibold text-text-primary">{event.title}</p><div className="flex items-center gap-2"><Badge color={tone.badge}>{event.channel}</Badge>{event.status === "current" && <Badge color="primary">Tiếp theo</Badge>}</div></div><p className="mt-1 text-sm leading-5 text-text-secondary">{event.description}</p></div></div></li>;
