...
export default function JourneyTimeline({ data }: Student360SectionProps) {
  ...
  return <Card className="flex min-w-0 flex-col p-5"><CardHeader className="mb-5"><div><CardTitle>Điểm chạm & hành trình</CardTitle><p className="mt-1 text-sm leading-6 text-text-tertiary">Toàn bộ điểm chạm đã ghi nhận trên biểu mẫu, website, sự kiện, Zalo và cuộc gọi.</p></div><Badge color="success">{completedCount} điểm chạm đã ghi nhận</Badge></CardHeader><ol className="flex flex-col" aria-label="Toàn bộ điểm chạm trong hành trình tuyển sinh">{data.journey.map((event) => {
