    ...
    <Card className="flex h-full flex-col p-5">
      ...
      <div className="mb-5 flex flex-col gap-3 rounded-lg border border-info-500/20 bg-badge-sky-background p-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-text-secondary">{completed}/{documents.length} tài liệu{deadline ? ` · Hạn hoàn tất: còn ${deadline.replace(/^Còn\s*/i, "")}` : ""}</p>
        </div>
        <div className="flex items-center gap-3 sm:min-w-60">
          <div className="h-2 flex-1 overflow-hidden rounded-full bg-card-background"><div className="h-full rounded-full bg-success-500" style={{ width: `${(completed / documents.length) * 100}%` }} /></div>
          <span className="text-sm font-semibold text-text-primary">{Math.round((completed / documents.length) * 100)}%</span>
        </div>
      </div>
        ...
        {documents.map((document) => {
          ...
          const row = (
            <div className="flex min-w-0 flex-1 items-start gap-3">
              ...
              <div className="min-w-0"><p className="truncate text-sm font-medium text-text-primary">{document.name}</p><p className="mt-1 text-xs text-text-tertiary">{document.type} · {document.date}</p></div>
            </div>
          );
