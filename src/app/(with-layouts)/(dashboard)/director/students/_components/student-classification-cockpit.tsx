  ...
  return (
    <Card className="mt-4 overflow-hidden border-primary-200/70 p-0 shadow-sm">
      <CardHeader className="border-b border-card-border px-5 py-4 lg:px-6">
        <div>
          <div className="flex flex-wrap items-center gap-2"><CardTitle>Vì sao cần chăm sóc học sinh này</CardTitle><Badge color={confirmed ? "success" : "warning"}>{confirmed ? <CheckCircle1 size={13} aria-hidden="true" /> : <InfoTriangle size={13} aria-hidden="true" />}{classification.reviewStatus}</Badge></div>
      ...
      <div className="flex flex-col gap-3 border-t border-card-border px-5 py-4 sm:flex-row sm:items-center sm:justify-between lg:px-6"><div className="flex min-w-0 items-start gap-2 text-xs leading-5 text-text-secondary"><Shield1Check size={16} className="mt-0.5 shrink-0 text-success-500" aria-hidden="true" /><span>{confirmed ? `Đã xác nhận bởi ${classification.reviewedBy}.` : `Chờ ${classification.reviewedBy} xác nhận.`}</span></div><div className="flex shrink-0 gap-2"><Button size="xs" appearance="outline" onPress={() => toast.success("Đã ghi nhận xác nhận phân loại.")}><CheckCircle1 size={14} />Xác nhận đúng</Button><Button size="xs" appearance="ghost" onPress={() => toast.info("Đã gửi yêu cầu điều chỉnh đánh giá.")}>Đánh giá lại</Button></div></div>
