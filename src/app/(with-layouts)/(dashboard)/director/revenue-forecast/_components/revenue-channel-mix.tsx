    ...
    <Card className="min-w-0 bg-background-gray-primary">
      <CardHeader className="items-start">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-text-tertiary">Đóng góp theo kênh tuyển sinh</p>
        </div>
        <span className="rounded-full bg-badge-primary-background px-2 py-1 text-[11px] font-semibold text-badge-primary-text">11.4K hồ sơ</span>
      </CardHeader>

      <div className="mt-4 h-44 w-full" aria-label="Biểu đồ cơ cấu nguồn thu theo kênh">
        <ChartContainer className="h-full w-full" height="100%" width="100%" minWidth={0} minHeight={0}>
          <BarChart data={CHANNEL_MIX} layout="vertical" margin={{ top: 0, right: 0, left: -8, bottom: 0 }}>
            ...
            />
            <Tooltip content={<RevenueChartTooltip valueSuffix="%" />} cursor={{ fill: "var(--background-gray-primary)", opacity: 0.5 }} />
            <Bar dataKey="value" name="Tỷ trọng" fill="var(--brand-500)" radius={[0, 4, 4, 0]} barSize={12} isAnimationActive={false} />
          </BarChart>
        </ChartContainer>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between border-t border-card-border pt-4 text-[11px]">
        ...
        <span className="font-semibold text-text-primary">Facebook Ads · 31%</span>
      </div>
    </Card>
  );
}
