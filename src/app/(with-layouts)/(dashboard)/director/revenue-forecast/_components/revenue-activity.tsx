...
export default function RevenueActivity() {
  ...
  const ACTIVITIES = activities.map((activity) => ({
    ...
    positive: activity.status === "received" || activity.status === "completed",
  }));
  return (
      ...
      <CardHeader className="items-start">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-text-tertiary">
            Các biến động doanh thu mới nhất
          </p>
        </div>
      ...
      <div className="mt-5 divide-y divide-card-border">
        {ACTIVITIES.map((activity) => {
          return (
            ...
            >
              <span
                className={`mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-full ${activity.positive ? "bg-badge-success-background text-badge-success-text" : "bg-badge-warning-background text-badge-warning-text"}`}
              >
                {activity.positive ? (
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  <p className="line-clamp-2 text-[11px] font-semibold leading-4 text-text-secondary">
                    {activity.title}
                  </p>
                  <span
                    className={`shrink-0 text-[11px] font-semibold ${activity.positive ? "text-success-500" : "text-warning-500"}`}
                  >
                    {activity.value}
                  </span>
                </div>
