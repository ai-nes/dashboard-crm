...
export default function ScenarioSimulation() {
  const { meta, scenarioSimulation, summary } = useRevenueForecastData();
  const revenueScenarios = scenarioSimulation.scenarios.map((scenario) => {
    const revenueValue = billions(scenario.revenue);
    const enrollmentValue =
      summary.forecastRevenue > 0
        ? Math.round(
            (summary.forecastEnrollment * scenario.revenue) /
              summary.forecastRevenue,
          )
        : summary.forecastEnrollment;
    return {
      ...scenario,
      description: "Mô phỏng từ snapshot hiện tại",
      enrollment: enrollmentValue.toLocaleString("vi-VN"),
      enrollmentValue,
      revenue: money(scenario.revenue),
      revenueValue,
      delta: "—",
      impact:
        scenario.id === scenarioSimulation.defaultScenarioId
          ? "Mốc dự báo hiện tại"
          : "Kịch bản điều chỉnh",
    };
  });
  const TARGET_REVENUE = billions(scenarioSimulation.targetRevenue);
  const [selectedScenario, setSelectedScenario] = useState(
    scenarioSimulation.defaultScenarioId,
  );
  const activeScenario =
    revenueScenarios.find((scenario) => scenario.id === selectedScenario) ??
    revenueScenarios[0];
  const baseScenario = revenueScenarios[0];
  const achievement = Math.round(
    (activeScenario.revenueValue / TARGET_REVENUE) * 100,
  );
  const remainingGap = Math.max(
    TARGET_REVENUE - activeScenario.revenueValue,
    0,
  );
  const additionalEnrollment =
    activeScenario.enrollmentValue - baseScenario.enrollmentValue;
  const additionalRevenue =
    activeScenario.revenueValue - baseScenario.revenueValue;
  ...
  const scenarioMeta = [
    ...
    {
      label: "Trạng thái",
      value: meta.status === "available" ? "Sẵn sàng" : "Một phần",
      note: "độ đầy đủ dữ liệu",
    },
  ];
  ...
  return (
    <Card className="flex h-full min-w-0 flex-col overflow-hidden bg-background-gray-primary">
      <CardHeader className="items-start">
        <div>
          ...
          <p className="mt-2 text-xs leading-5 text-text-tertiary">
            Thử các đòn bẩy tăng trưởng và xem ngay tác động đến chỉ tiêu
          </p>
        </div>
      ...
      <div
        className="mt-5 grid grid-cols-1 items-stretch gap-2 sm:grid-cols-3"
        role="group"
        ...
        {revenueScenarios.map((scenario) => {
          ...
          return (
            <Button
              ...
              className={
                isSelected
                  ? "h-[68px] min-h-0 w-full justify-start text-left"
                  : "h-[68px] min-h-0 w-full justify-start border-card-border bg-card-background text-left"
              }
            ...
            >
              <span className="min-w-0">
                ...
                <span
                  className={`mt-1 block truncate text-[11px] ${isSelected ? "text-white-80" : "text-text-tertiary"}`}
                >
                  {scenario.impact}
                </span>
              </span>
      ...
      <div className="mt-5 grid min-w-0 grid-cols-1 gap-4 xl:grid-cols-[minmax(250px,0.8fr)_minmax(0,1.2fr)]">
        <div className="rounded-2xl bg-card-background p-5">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <h3 className="mt-2 truncate text-lg font-semibold text-text-primary">
            ...
            <div className="shrink-0 text-right">
              <span className="inline-flex items-center gap-1 text-sm font-semibold text-success-500">
                {additionalRevenue ? (
                  <ArrowUpward size={14} aria-hidden="true" />
                ) : (
                  <CheckCircle1 size={14} aria-hidden="true" />
                )}
                {additionalRevenue
                  ? `+${formatBillions(additionalRevenue)}`
                  : "Giữ nguyên"}
              </span>
          ...
          <div className="mt-6 grid grid-cols-2 gap-x-4 gap-y-5">
            <ScenarioMetric
              label="Nhập học dự kiến"
              value={activeScenario.enrollment}
              note="người"
            />
            <ScenarioMetric
            ...
            <ScenarioMetric
              label="Tăng thêm"
              value={`+${additionalEnrollment}`}
              note="người so với cơ sở"
              tone="text-success-500"
            />
            <ScenarioMetric
              label="Còn thiếu"
              value={formatBillions(remainingGap)}
              note={`${achievement}% chỉ tiêu`}
              tone={
                remainingGap ? "text-badge-warning-text" : "text-success-500"
              }
            />
          </div>
          ...
          <div className="mt-6 border-t border-card-border pt-4">
            <div className="mb-2 flex items-center justify-between text-[11px] text-text-tertiary">
              <span>
                Tiến tới mục tiêu {money(scenarioSimulation.targetRevenue)}
              </span>
        ...
        <div className="min-w-0 rounded-2xl border border-card-border bg-card-background p-4">
              ...
              >
                <XAxis
                  ...
                  tickFormatter={(value: string) =>
                    value === "Cơ sở"
                      ? "Cơ sở"
                      : value.replace("Tỷ lệ chuyển đổi ", "")
                  }
                />
                <YAxis hide domain={[0, Math.max(TARGET_REVENUE * 1.1, 1)]} />
                <ReferenceLine
                  y={TARGET_REVENUE}
                ...
                <Tooltip
                  ...
                  cursor={{
                    fill: "var(--background-gray-primary)",
                    opacity: 0.5,
                  }}
                />
                <Bar
          ...
          <div className="mt-2 flex flex-wrap items-center gap-4 text-[11px] text-text-tertiary">
            ...
            <span className="flex items-center gap-1.5">
              <span
                className="h-px w-4 border-t border-dashed border-text-tertiary"
                aria-hidden="true"
              />
              Mục tiêu
            </span>
...
function ScenarioMetric({
  ...
  return (
    <div className="min-w-0">
      ...
      <p
        className={`mt-1 truncate text-xl font-semibold tracking-[-0.6px] ${tone}`}
      >
        {value}
      </p>
...
function ScenarioMeta({
  ...
  return (
    <div className="h-[82px] min-w-0 rounded-xl bg-card-background px-3 py-2.5">
      ...
      <p className={`mt-1 truncate text-sm font-semibold ${accent.value}`}>
        {value}
      </p>
      <p className="mt-0.5 line-clamp-2 text-[10px] leading-4 text-text-tertiary">
        {note}
      </p>
    </div>
  );
