...
import { billions, useRevenueForecastData } from "./revenue-forecast-context";

export default function RevenueForecastChart() {
  const { forecast, summary } = useRevenueForecastData();
  const revenueForecast = forecast.points.map((point) => ({
    ...point,
    actual: point.actual == null ? null : billions(point.actual),
  ...
  return (
    <Card className="min-w-0 overflow-hidden">
      <CardHeader className="mb-4 items-start">
        ...
        <span className="flex items-center gap-1.5 rounded-full bg-badge-success-background px-2.5 py-1 text-xs font-semibold text-badge-success-text">
          ...
          {summary.changeVsPrevious == null
            ? "Chưa có so sánh kỳ trước"
            : `${summary.changeVsPrevious}% so với kỳ trước`}
        </span>
      </CardHeader>

      <div
        className="relative h-[280px] w-full sm:h-[320px] xl:h-[360px]"
        aria-label="Biểu đồ dự báo doanh thu theo niên khóa"
      >
        <ChartContainer
      ...
      <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-text-tertiary">
        ...
        <LegendItem
          label="Chỉ tiêu"
          className="border-t border-dashed border-text-tertiary"
        />
      </div>
    </Card>
  );
