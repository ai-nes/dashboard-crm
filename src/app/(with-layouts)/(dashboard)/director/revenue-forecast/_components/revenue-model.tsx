...
export default function RevenueModel() {
  const { model, summary } = useRevenueForecastData();
  const revenueModel = [
    {
      id: "tuition",
    ...
    },
    {
      id: "scholarship",
      label: "Học bổng",
      value: money(model.scholarship),
      amount: billions(model.scholarship),
      note: "giảm trừ",
      tone: "warning" as RevenueMetricTone,
    },
    {
      id: "discount",
      label: "Chiết khấu & miễn giảm",
      value: money(model.discount),
      amount: billions(model.discount),
      note: "giảm trừ",
      tone: "danger" as RevenueMetricTone,
    },
  ];
  const MODEL_DONUT = [
    {
    ...
    {
      id: "reductions",
      label: "Học bổng & chiết khấu",
      value: billions(model.scholarship + model.discount),
    ...
    <Card className="flex min-w-0 flex-col bg-background-gray-primary">
        ...
        <span className="rounded-full bg-badge-success-background px-2.5 py-1 text-xs font-semibold text-badge-success-text">
          {percent(
            summary.revenueTarget
              ? (model.netRevenue / summary.revenueTarget) * 100
              : null,
          )}{" "}
          chỉ tiêu
        </span>
      ...
      <div className="relative h-52">
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-3xl font-semibold tracking-[-1px] text-text-primary">
        ...
        {revenueModel.slice(0, 3).map((item) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`flex shrink-0 items-center gap-1 font-semibold ${tone.text}`}
              >
                {item.id !== "tuition" && (
                  <ArrowDownward size={12} aria-hidden="true" />
                )}
                {item.value}
              </span>
            </div>
