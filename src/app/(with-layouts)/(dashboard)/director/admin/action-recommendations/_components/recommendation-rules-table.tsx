...
export default function RecommendationRulesTable({
  canEdit,
  ...
  const query = useNbaRulesQuery({
    status:
      status === "all"
        ? undefined
        : (status as NbaRecommendationRule["status"]),
  ...
  const rules = useMemo(
    () =>
      (query.data?.rules ?? []).filter(
        (rule) => priority === "all" || rule.priority === priority,
      ),
    [priority, query.data?.rules],
  );
  ...
  return (
    <>
      <AdminTableFrame aria-labelledby="recommendation-rules-heading">
        <div className="flex flex-wrap items-end justify-between gap-3 px-4 py-4">
          <div>
            <h2
              id="recommendation-rules-heading"
              className="text-base font-semibold text-text-primary"
            >
              Quy tắc đề xuất
            </h2>
            <p className="mt-1 max-w-2xl text-sm text-text-secondary">
              Xác định khi nào hệ thống nên gợi ý một việc phù hợp cho hồ sơ.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs text-text-tertiary">
              {query.isFetching ? "Đang tải…" : `${rules.length} quy tắc`}
            </span>
            {canEdit && (
              <Button size="sm" onPress={() => openRule(null)}>
        ...
        <div className="flex items-start gap-2 border-y border-card-border bg-background-gray-secondary_alt px-4 py-3 text-xs leading-5 text-text-secondary">
          <span
            className="mt-1 size-1.5 shrink-0 rounded-full bg-primary-500"
            aria-hidden="true"
          />
          Quy tắc đã phát hành sẽ chuyển về bản nháp và tắt khi được chỉnh sửa.
        </div>
        <AdminTableToolbar
          search={search}
          onSearchChange={setSearch}
          searchLabel="Tìm quy tắc đề xuất"
          searchPlaceholder="Tìm theo mã, tên hoặc mô tả"
          filters={[
            {
              label: "Lọc theo trạng thái quy tắc",
              value: status,
        ...
        {query.error ? (
          <div className="px-5 py-10 text-center" role="alert">
            <p className="text-sm font-medium text-alert-danger-title">
              Không tải được quy tắc đề xuất.
            </p>
          ...
          <p className="px-5 py-12 text-center text-sm text-text-tertiary">
            Đang tải quy tắc đề xuất…
          </p>
        ) : (
          <AdminTableRoot>
            <AdminTableHeader>
              <AdminTableRow>
                <AdminTableHead>Quy tắc</AdminTableHead>
                <AdminTableHead>Hành động</AdminTableHead>
                <AdminTableHead>Kích hoạt</AdminTableHead>
                <AdminTableHead>Điều kiện</AdminTableHead>
            ...
            <TableBody>
              {rules.map((rule) => (
                <AdminTableRow key={rule.name}>
                  <AdminTableCell className="min-w-64">
                    ...
                    <p className="mt-1 font-semibold">{rule.displayName}</p>
                    <p className="mt-1 max-w-sm truncate text-xs leading-5 text-text-secondary">
                      {rule.description || "Chưa có mô tả"}
                    </p>
                  </AdminTableCell>
                  <AdminTableCell className="min-w-40">
                    <p className="font-medium">
                      {actionLabels.get(rule.actionCode) ?? rule.actionCode}
                    </p>
                    <p className="mt-1 font-mono text-[11px] text-text-tertiary">
                      {rule.actionCode || "Chưa chọn"}
                    </p>
                  </AdminTableCell>
                  <AdminTableCell className="min-w-40">
                    <p className="text-text-secondary">{rule.triggerType}</p>
                    <p className="mt-1 text-xs text-text-tertiary">
                      {rule.triggerEvent || "Không gắn sự kiện"}
                    </p>
                  </AdminTableCell>
                  <AdminTableCell className="whitespace-nowrap text-text-secondary">
                    {rule.conditions.all.length + rule.conditions.any.length}{" "}
                    điều kiện
                  </AdminTableCell>
                  <AdminTableCell>
                  ...
                  <AdminTableCell className="min-w-36">
                    <p className="text-text-secondary">
                      {rule.timingPolicy || "Không đặt"}
                    </p>
                    <p className="mt-1 text-xs text-text-tertiary">
                      Tối đa {rule.maxOccurrences} lần
                    </p>
                  </AdminTableCell>
                  <AdminTableCell>
                    <div className="flex flex-col items-start gap-1">
                      ...
                      <span className="text-[11px] text-text-tertiary">
                        v{rule.version}
                        {rule.enabled ? " · Đang dùng" : " · Tạm dừng"}
                      </span>
                    </div>
                  </AdminTableCell>
                  <AdminTableCell className="text-right">
                    <Button
                      ...
                      aria-label={`${canEdit ? "Chỉnh sửa" : "Xem"} quy tắc ${rule.displayName}`}
                      className="min-w-20"
                    >
                      {canEdit ? (
              ...
              {rules.length === 0 && (
                <AdminTableRow>
                  ...
                  >
                    Không tìm thấy quy tắc đề xuất phù hợp.
                  </AdminTableCell>
                </AdminTableRow>
              )}
            </TableBody>
