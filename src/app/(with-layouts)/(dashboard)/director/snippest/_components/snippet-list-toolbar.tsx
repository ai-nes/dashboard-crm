...
import type { SnippetOwner } from "@/services/api/snippets";
...
export default function SnippetListToolbar({
  ...
  const selectedOwner = owners.find((ownerOption) => ownerOption.id === owner);
  const hasFilter =
      ...
      <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2.5">
        <InputGroup className="h-10 w-full sm:w-[26rem]">
          ...
          <InputGroupInput
            ...
            placeholder="Tìm theo mã, internal name, shortcut hoặc nội dung..."
            value={search}
        ...
        <div
          role="group"
          aria-label="Phạm vi và chủ sở hữu snippet"
            ...
            >
              ...
              <SelectValue className="flex min-w-0 items-center gap-2">
                <span className="max-w-40 truncate">
                  {owner === "all" ? "Tất cả" : selectedOwner?.name || owner}
                </span>
                {owner === "all" ? (
