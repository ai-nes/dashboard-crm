...
export default function SchoolOutcomes({ data }: SchoolOutcomesProps) {
  const [hoveredSegmentKey, setHoveredSegmentKey] =
    useState<ScoreBandKey | null>(null);
  ...
  const centerSegment =
    scoreDistribution.find((segment) => segment.key === hoveredSegmentKey) ??
    scoreDistribution[1];
      ...
      <CardHeader className="border-b border-card-border p-5 pb-4 lg:p-6 lg:pb-4">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-text-tertiary">
            Xác định nhóm học sinh nên ưu tiên tư vấn
          </p>
        </div>
      </CardHeader>
      ...
      <div className="grid min-w-0 gap-6 p-5 lg:grid-cols-2 lg:p-6">
              ...
              <PieChart>
                ...
                <Pie
                  ...
                  onMouseEnter={(_, index) =>
                    setHoveredSegmentKey(scoreDistribution[index]?.key ?? null)
                  }
            ...
            <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
              <p className="text-2xl font-semibold text-text-primary">
                {hasScoreDistribution
                  ? centerSegment.students.toLocaleString("vi-VN")
                  : "-"}
              </p>
              <p className="mt-0.5 max-w-28 text-center text-xs leading-4 text-text-tertiary">
                {hoveredSegmentKey ? centerSegment.label : "HS phù hợp"}
              </p>
              {hoveredSegmentKey ? (
                <p className="mt-0.5 text-xs font-medium text-text-secondary">
                  {hasScoreDistribution
                    ? `${centerSegment.share}% tổng số`
                    : "-"}
                </p>
              ) : null}
            </div>
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
            {scoreDistribution.map((segment) => (
              <div key={segment.key} className="min-w-0">
                ...
                <p className="mt-1 pl-4 text-sm font-semibold text-text-primary">
                  {hasScoreDistribution
                    ? segment.students.toLocaleString("vi-VN")
                    : "-"}{" "}
                  <span className="font-normal text-text-tertiary">
                    ({hasScoreDistribution ? `${segment.share}%` : "-"})
                  </span>
                </p>
              </div>
            ))}
        ...
        <div className="min-w-0 lg:border-l lg:border-card-border lg:pl-6">
          <div className="flex items-center justify-between gap-3">
            ...
            <Badge color="warning">
              Cạnh tranh {data.geography.competitionDensity.toLowerCase()}
            </Badge>
          </div>
            ...
            >
              ...
              >
                ...
                <Bar
                  ...
                  barSize={18}
                >
                  {chartChoices.map((item, index) => (
                    <Cell
                      key={`${item.label}-${index}`}
                      fill={CHOICE_COLORS[index]}
                    />
                  ))}
                  <LabelList
                    dataKey="share"
                    ...
                    fontSize={11}
                    formatter={(value) =>
                      hasChoiceBreakdown ? String(value) + "%" : "-"
                    }
                  />
                </Bar>
              </BarChart>
          ...
          <div className="mt-4 grid gap-2 border-t border-card-border pt-4 sm:grid-cols-3 lg:grid-cols-1 2xl:grid-cols-3">
            <ContextItem
              label="Đối thủ chính"
              value={data.competitionContext.leadingChoice}
            />
            <ContextItem
              label="Lý do chọn khác"
              value={data.competitionContext.lostReason}
            />
            <ContextItem
              label="Hoạt động ngoài trường"
              value={data.competitionContext.externalPresence}
            />
          </div>
        </div>
      </div>
...
function ContextItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0 rounded-xl border border-card-border bg-background-soft-50 px-3 py-2.5">
...
function ChoiceTooltip({
  ...
  active?: boolean;
  payload?: { payload?: SchoolChoiceBreakdown }[];
}) {
  ...
  return (
    <div className="rounded-xl border border-card-border bg-card-background p-3 text-xs shadow-theme-md">
      <p className="max-w-48 font-semibold text-text-primary">{item.label}</p>
      <p className="mt-1 text-text-secondary">
        {item.students.toLocaleString("vi-VN")} học sinh · {item.share}%
      </p>
    </div>
  );
}
