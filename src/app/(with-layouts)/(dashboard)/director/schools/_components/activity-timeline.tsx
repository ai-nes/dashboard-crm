  ...
  return (
    <Card className="min-w-0 p-5">
      <CardHeader className="mb-5">
        <div><CardTitle>Hoạt động gần đây</CardTitle><p className="mt-1 text-xs leading-5 text-text-tertiary">Lịch sử tương tác và hành động sắp tới tại trường.</p></div>
      </CardHeader>
        ...
        {data.activities.map((activity, index) => (
          <li key={activity.id} className="relative flex gap-3 pb-5 last:pb-0">
            ...
            <div className="min-w-0 pt-0.5">
              <div className="flex flex-wrap items-center gap-2"><p className="text-sm font-medium text-text-primary">{activity.title}</p><Badge color={activity.status === "scheduled" ? "primary" : "success"}>{activity.status === "scheduled" ? "Đã lên lịch" : "Hoàn thành"}</Badge></div>
              <p className="mt-1 flex items-center gap-1.5 text-xs text-text-secondary"><ClockThree size={13} />{activity.date}</p>
              <p className="mt-1 text-xs text-text-tertiary">{activity.owner}</p>
            </div>
          </li>
        ))}
      </ol>
