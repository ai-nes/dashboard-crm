...
import type { StudentStatusDataItem } from "./data";
...
export default function StudentStatusChart({
  ...
  items: StudentStatusDataItem[];
  total: number;
}) {
  const studentStatusData = items;
  ...
  return (
    <Card className="min-w-0 p-5 sm:p-6">
      <CardHeader className="items-start">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-text-tertiary">
            Phân bổ {totalStudents} học sinh của cả đội.
          </p>
        </div>
      ...
      <div
        ...
        role="img"
        aria-label={`Biểu đồ trạng thái ${totalStudents} học sinh`}
      >
        <ChartContainer
