...
import type { TeamPerformance as TeamPerformanceItem } from "./data";
  ...
  return (
    <Card className="min-w-0 p-5 sm:p-6">
      <CardHeader className="items-start">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-text-tertiary">
            So sánh tiến độ xử lý của từng thành viên.
          </p>
        </div>
        <Link
      ...
      <div className="mt-5 hidden overflow-x-auto xl:block">
        <table
          className="w-full min-w-[38rem] table-fixed border-collapse text-sm"
          aria-label="Bảng hiệu suất đội ngũ Sale"
        >
          <colgroup>
            <col className="w-[32%]" />
            <col className="w-[15%]" />
          ...
          <thead>
            <tr className="border-b border-card-border text-[10px] font-medium text-text-tertiary">
              <th
                scope="col"
                className="px-2 pb-3 text-left font-medium whitespace-nowrap"
              >
                Thành viên
              </th>
              <th
                scope="col"
                className="px-2 pb-3 text-center font-medium whitespace-nowrap"
              >
                Đang phụ trách
              </th>
              <th
          ...
          <tbody className="divide-y divide-card-border">
            {items.map((member) => (
              <tr key={member.id}>
                ...
                <td className="px-2 py-3.5 text-center font-semibold text-text-primary">
                  {member.activeStudents}
                </td>
                <td className="px-2 py-3.5 text-center font-semibold text-text-primary">
                  {member.consulted}
                </td>
                <td className="px-2 py-3.5 text-center font-semibold text-success-500">
                  {member.admitted}
                </td>
                <td className="px-2 py-3.5 text-center">
                  <Badge
                    color={member.status === "Tốt" ? "success" : "warning"}
                    size="sm"
                    className="text-[10px]"
                  >
                    {member.status}
      ...
      <div className="mt-5 space-y-3 xl:hidden">
        {items.map((member) => (
          <div
            key={member.id}
            className="rounded-xl border border-card-border bg-background-soft-50 p-3"
          >
            ...
            <div className="mt-3 grid grid-cols-3 gap-2">
              ...
              <div className="rounded-lg bg-card-background px-2 py-1.5">
                ...
                <p className="mt-0.5 text-sm font-semibold text-success-500">
                  {member.admitted}
                </p>
              </div>
            </div>
