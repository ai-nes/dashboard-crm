...
export default function AssignmentHistory() {
  ...
  const table = useReactTable({
    data: records,
    columns: historyColumns(inspect),
      ...
      <Card className="overflow-hidden p-0">
        ...
        <div className="hidden overflow-x-auto xl:block">
          <table className="w-full table-fixed text-left text-sm">
            ...
            <colgroup><col className="w-[30%]" /><col className="w-[24%]" /><col className="w-[24%]" /><col className="w-[10%]" /><col className="w-[12%]" /></colgroup>
            <thead className="border-y border-card-border bg-background-gray-secondary/60 text-xs text-text-tertiary">
              {table.getHeaderGroups().map((group) => (
                <tr key={group.id}>{group.headers.map((header) => (
                  <th key={header.id} scope="col" className="px-4 py-3 font-medium whitespace-nowrap first:pl-5">{flexRender(header.column.columnDef.header, header.getContext())}</th>
                ))}</tr>
              ))}
            </thead>
            <tbody className="divide-y divide-card-border">
              {rows.map((row) => <tr key={row.id}>{row.getVisibleCells().map((cell) => <td key={cell.id} className="px-4 py-4 align-middle first:pl-5">{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>)}</tr>)}
            </tbody>
          </table>
        </div>
        <div className="divide-y divide-card-border border-t border-card-border xl:hidden">
          {rows.map(({ original: record }) => (
            <div key={record.id} className="p-4">
              <div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-semibold text-text-primary">{record.name}</p><span className="text-xs text-text-tertiary">{record.time}</span></div>
              <p className="mt-1 text-xs text-text-tertiary">{record.school}</p>
              <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                <Badge color={statusColors[record.status]}>{statusLabels[record.status]}</Badge>
                <Button appearance="ghost" size="sm" className="text-text-secondary" aria-label={`Xem chi tiết ${record.name}`} onPress={() => inspect(record.id)}>Chi tiết <ArrowRight size={14} /></Button>
              </div>
            </div>
          ))}
        </div>
        {!rows.length && (
          <div className="px-5 py-10 text-center">
            ...
            <p className="mt-3 text-sm font-medium text-text-primary">{isLoading ? "Đang tải dữ liệu…" : "Không có học sinh phù hợp"}</p>
            {!isLoading && <><p className="mt-1 text-xs text-text-tertiary">Thử đổi từ khóa hoặc bỏ bộ lọc.</p><Button appearance="ghost" size="sm" className="mx-auto mt-3" onPress={() => { setQuery(""); setFilter("all"); }}>Xóa bộ lọc</Button></>}
          </div>
        )}
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-card-border px-5 py-3">
          <p className="text-xs text-text-tertiary" aria-live="polite">{isFetching ? "Đang đồng bộ…" : total ? `${(currentPage - 1) * pageSize + 1}–${Math.min(currentPage * pageSize, total)} trong ${total} học sinh` : "0 học sinh"}</p>
          <div className="flex items-center gap-2">
            <Button appearance="outline" size="sm" iconOnly aria-label="Trang trước" className="border-card-border text-text-secondary" isDisabled={currentPage <= 1 || isFetching} onPress={() => setPage(currentPage - 1)}><ArrowLeft size={16} /></Button>
            <span className="min-w-12 text-center text-xs tabular-nums text-text-secondary">{currentPage} / {totalPages}</span>
            <Button appearance="outline" size="sm" iconOnly aria-label="Trang sau" className="border-card-border text-text-secondary" isDisabled={!pagination?.hasNextPage || isFetching} onPress={() => setPage(currentPage + 1)}><ArrowRight size={16} /></Button>
          </div>
        </div>
      </Card>
    </section>
