...
export function findMember(members: TeamMember[], id: string | null): TeamMember | null {
  if (!id) return null;
  return members.find((item) => item.id === id) ?? null;
}

export function smallTeamsOfBigTeam(state: TeamOrgState, bigTeam: BigTeam): SmallTeam[] {
  return state.smallTeams.filter((team) => bigTeam.smallTeamIds.includes(team.id));
}

export function membersOfSmallTeam(state: TeamOrgState, smallTeam: SmallTeam): TeamMember[] {
  return smallTeam.memberIds
    .map((id) => findMember(state.members, id))
    .filter((item): item is TeamMember => item !== null);
}

export function membersOfBigTeam(state: TeamOrgState, bigTeam: BigTeam): TeamMember[] {
...
export function unassignedMembers(state: TeamOrgState): TeamMember[] {
  const assignedIds = new Set(state.smallTeams.flatMap((team) => team.memberIds));
