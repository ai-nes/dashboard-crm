...
import { RevenueOverviewStatSkeleton } from "./skeleton";
import type { RevenueOverviewViewModel } from "./types";

type HeaderSectionProps = {
  isLoading: boolean;
  chartData?: RevenueOverviewViewModel;
  stats: Array<{
    id: string;
    label: string;
    value: string;
...
export default function HeaderSection({ isLoading, stats, chartData }: HeaderSectionProps) {
  return (
    <CardHeader className="mb-6 flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-center">
      <CardTitle className="sr-only">Revenue Overview</CardTitle>

      <div className="flex flex-col gap-8 sm:flex-row">
        {isLoading || !chartData ? (
          <>
            <RevenueOverviewStatSkeleton />
            <RevenueOverviewStatSkeleton />
          </>
        ) : (
          stats.map((stat) => (
