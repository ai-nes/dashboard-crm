...
const STATUS_FILTERS: Array<{
  value: AdmissionProfileTemplateStatus | "all";
  label: string;
}> = [
...
function errorMessage(error: unknown): string {
  return error instanceof Error
    ? error.message
    : "Không thể tải danh sách loại hồ sơ.";
}

function templateKindLabel(
  kind: AdmissionProfileTemplateOption["templateKind"],
): string {
...
function FilterSelect({
  ...
  return (
    ...
    >
      <SelectTrigger
        size="sm"
        className="min-w-36 justify-between whitespace-nowrap"
      >
        <SelectValue />
        <SelectIndicator />
      </SelectTrigger>
...
export function AdmissionProfileTemplateManagement({
  ...
  const [templateToDelete, setTemplateToDelete] =
    useState<AdmissionProfileTemplateOption | null>(null);

  const templates = query.data?.templates ?? EMPTY_TEMPLATES;
  const methodLabels = useMemo(
    () =>
      new Map(
        (methodsQuery.data?.methods ?? []).flatMap((method) => [
  ...
  const changeStatus = async (
    ...
    try {
      ...
      toast.success(
        `Đã chuyển ${template.name} sang ${STATUS_LABELS[status]}.`,
      );
    } catch (error) {
  ...
  return (
    <>
      ...
      >
        <p className="text-sm text-text-secondary">
          Chỉ bản nháp chưa được hồ sơ tuyển sinh sử dụng mới được xóa. Template
          đang dùng nên chuyển sang lưu trữ.
        </p>
      </DeleteRecordDialog>
    </>
  );
