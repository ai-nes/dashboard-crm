...
function GroupEditor({
  ...
  return (
    <Backdrop isOpen onOpenChange={(open) => !open && !isSaving && onClose()}>
      <Dialog
        aria-label={
          group ? `Chỉnh sửa nhóm ${group.label}` : `Tạo nhóm ${title}`
        }
      ...
      >
        <DialogHeader className="border-b border-card-border px-5 py-4 pr-12">
          <DialogTitle>
            {group ? "Chỉnh sửa nhóm" : `Tạo nhóm ${title}`}
          </DialogTitle>
...
function TermEditor({
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : `Không thể lưu ${childLabel}.`,
      );
    }
  };
  ...
  return (
    <Backdrop isOpen onOpenChange={(open) => !open && !isSaving && onClose()}>
      <Dialog
        aria-label={term ? `Chỉnh sửa ${childLabel}` : `Tạo ${childLabel}`}
        className="max-h-[calc(100vh-2rem)] max-w-xl overflow-hidden p-0"
      >
        <DialogHeader className="border-b border-card-border px-5 py-4 pr-12">
          <DialogTitle>
        ...
        <DialogBody className="space-y-4 overflow-y-auto px-5 py-5">
          <label className="block space-y-1.5">
            ...
            <Input
              ...
              disabled={Boolean(term) || isSaving}
              placeholder="Ví dụ: TUITION_INFORMATION"
              className="h-10 w-full"
            />
            <span className="text-xs text-text-tertiary">
              Mã không thể thay đổi sau khi tạo.
          ...
          <label className="block space-y-1.5">
            ...
            <Input
              ...
              disabled={isSaving}
              placeholder={`Ví dụ: ${childLabel} cần tư vấn`}
              className="h-10 w-full"
            />
          </label>
          <label className="block space-y-1.5">
            ...
            <select
              ...
              className="h-10 w-full rounded-lg border border-card-border bg-input-background px-3 text-sm text-title-50 outline-none focus:ring-4 focus:ring-input-primary-focus-border/20"
            >
              {groups.map((group) => (
...
export function ClassificationGroupManagement({
  ...
  const [toDelete, setToDelete] = useState<ClassificationGroupRecord | null>(
    null,
  );
  const [termToEdit, setTermToEdit] = useState<
    SegmentTermRecord | null | undefined
  >(undefined);
  ...
  const changeTermStatus = async (
    ...
    try {
      ...
      toast.success(
        `Đã chuyển trạng thái ${childLabel} sang ${STATUS_LABELS[status]}.`,
      );
    } catch (error) {
      toast.error(
        error instanceof Error
          ? error.message
          : `Không thể cập nhật trạng thái ${childLabel}.`,
      );
    }
  };
    ...
    try {
      await deleteTermMutation.mutateAsync({
        ...
        payload: {
          ...
          expectedRevision: termToDelete.revision ?? 0,
        },
      });
      ...
      toast.error(
        error instanceof Error ? error.message : `Không thể xóa ${childLabel}.`,
      );
    }
  };

  const terms =
    (kind === "need" ? termsQuery.data?.needs : termsQuery.data?.tags) ?? [];
  const selectedGroup =
    groups.find((group) => group.name === effectiveGroupName) ?? groups[0];
  const selectedGroupTerms = terms;
  const groupTotal = query.data?.total ?? 0;
  const termTotal = selectedGroup ? (termsQuery.data?.total ?? 0) : 0;
  const groupTotalPages = Math.max(1, Math.ceil(groupTotal / PAGE_SIZE));
  const termTotalPages = Math.max(1, Math.ceil(termTotal / PAGE_SIZE));
  ...
  return (
    <>
      <section
        className={`flex h-full min-h-0 flex-col overflow-hidden bg-card-background ${hideHeader ? "rounded-none border-0 shadow-none" : "rounded-2xl border border-card-border shadow-xs"}`}
      >
        {!hideHeader && (
                ...
                <div className="shrink-0 border-b border-card-border bg-background-gray-secondary/25 px-5 py-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <h3
                        id={`selected-group-${selectedGroup.name}`}
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      {canManage ? (
                        <SegmentStatusSelect
                          ...
                          onChange={(status) =>
                            void changeStatus(selectedGroup, status)
                          }
                        />
                      ) : (
                        <Badge
                      ...
                      {canManage && (
                        <Button
                          ...
                          appearance="ghost"
                          variant="danger"
                          onPress={() => setToDelete(selectedGroup)}
                        >
      ...
      >
        <p className="text-sm text-text-secondary">
          Nếu {childLabel} đang được học sinh hoặc segment sử dụng, backend sẽ
          chặn xóa và yêu cầu lưu trữ.
        </p>
      </DeleteRecordDialog>
    </>
  );
